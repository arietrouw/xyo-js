...
export class XyoServerTcpNetwork extends XyoBase {
  ...
  constructor(port: number) {
    ...
    this.server.on('error', (e) => {
      this.log.warn(`Unknown server socket error: ${e}`)

      if (!this.server.listening) {
        this.log.info(`Starting listing again on port ${this.port}`)
  ...
  private connectionListener(socket: net.Socket) {
    this.log.info(`New connection made with ${socket.remoteAddress}:${socket.remotePort}`)

    socket.on('error', (e) => {
      this.log.warn(`Unknown socket error: ${e}`)
      socket.destroy()
    })
