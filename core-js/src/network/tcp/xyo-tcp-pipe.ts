...
export class XyoTcpPipe extends XyoBase implements XyoNetworkPipe {
  ...
  public async send(data: Buffer, waitForResponse: boolean): Promise<Buffer | undefined> {
    this.log.verbose(`Sending data through socket: ${data.toString('hex')}`)
    await this.sendData(data)

    if (waitForResponse) {
      const response = await this.waitForMessage()
      this.log.verbose(`Got data through socket: ${response.toString('hex')}`)
      return response
    }
  ...
  public async close() {
    this.log.info(`Closing connection with ${this.socket.remoteAddress}:${this.socket.remotePort}`)
    this.socket.end()
