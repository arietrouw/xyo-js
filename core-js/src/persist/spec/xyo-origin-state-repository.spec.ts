...
function testOriginStateRepository(repo: XyoOriginStateRepository) {
  describe('IXyoOriginStateRepository interface', () => {
    it('Should validate', async () => {
      ...
      expect(repo.getPreviousHash()?.getAll().getContentsCopy().toString('hex')).toBe(
        testPreviousHash0?.getAll().getContentsCopy().toString('hex')
      )
      ...
      expect(repo.getPreviousHash()?.getAll().getContentsCopy().toString('hex')).toBe(
        testPreviousHash1?.getAll().getContentsCopy().toString('hex')
      )
      expect(repo.getIndex()?.getAll().getContentsCopy().toString('hex')).toBe(
