...
export class XyoSecp2556k1 implements XyoSigner {
  ...
  private static buildDerSignature(xyBuffer: Buffer) {
    ...
    return Buffer.concat([Buffer.from([0x30]), sourceBufferSizeBuffer, source]).toString('hex')
  }
  ...
  public sign(data: Buffer): XyoStructure {
    ...
    const value = Buffer.concat([Buffer.from([rBuffer.length]), rBuffer, Buffer.from([sBuffer.length]), sBuffer])
  ...
  public getPublicKey(): XyoStructure {
    ...
    const buffer = Buffer.concat([this.writePointTo32ByteBuffer(x), this.writePointTo32ByteBuffer(y)])
