...
export class XyoBoundWitnessInserter extends XyoBase {
  ...
  constructor(hasher: XyoHasher, state: XyoOriginState, blockRepo: XyoOriginBlockRepository) {
    super()
    this.hasher = hasher
    this.state = state
  ...
  public async insert(boundWitness: XyoBoundWitness) {
    ...
    this.log.info(`Inserted new origin block with hash: ${bs58.encode(hash.getAll().getContentsCopy())}`)
    ...
    this.log.info(`Origin state at new height: ${this.state.getIndexAsNumber()}`)
