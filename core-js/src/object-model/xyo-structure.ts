...
export class XyoStructure {
  ...
  constructor(contents: XyoBuffer | Buffer, overrideSchema?: XyoSchema) {
    this.contents = contents instanceof Buffer ? (this.contents = new XyoBuffer(contents)) : contents
  ...
  public getAll(): XyoBuffer {
    if (this.overrideSchema) {
      ...
      const together = Buffer.concat([headerBuffer, this.contents.getContentsCopy()])
