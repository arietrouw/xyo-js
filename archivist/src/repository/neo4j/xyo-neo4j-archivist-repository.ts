...
export class XyoArchivistNeo4jRepository extends XyoBase implements XyoOriginBlockGetter, XyoOriginBlockRepository {
  ...
  public async getOriginBlocksByPublicKey(_publicKey: Buffer): Promise<{ items: Buffer[]; total: number }> {
  ...
  public async addOriginBlocks(_hashes: Buffer, _blocks: Buffer): Promise<void> {
    return
  }
  ...
  public async addOriginBlock(_hash: Buffer, _originBlock: Buffer): Promise<void> {
    return
  }
  ...
  public async getBlocksThatProviderAttribution(_hash: Buffer): Promise<{ [h: string]: Buffer }> {
    return {}
  }
